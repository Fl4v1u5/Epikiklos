...
with_jquery(function($) { $(function () {
    ...
    function homeHandler(shiftKey) {
        ...
        if (!lastLine.length) { // we're at the beginning of the line
            ...
            // if this line isn't indente by at least four spaces or one tab, fall back to native behavior,
            // since that can correctly handle soft line wrapping in the textarea, which we can't
            if (firstRealCharPos > 3 || (firstRealCharPos > 0 && /^ {0,3}\t/.test(lastLine)))
    ...
    function tabHandler (shiftKey) {
        ...
        if (selection.start === selection.end || !/\n/.test(selMarkdown)) {
            ...
            if (shiftKey) {
                var removeCount = lastLine.length ? skew % 4 || 4 : 4, // 1 to 4
                ...
                addCount = 4 - (skew % 4);
                preSelMarkdown += new Array(addCount + 1).join(" ") ;
            }
        ...
        } else { // it's a selection that spans multiple lines
            ...
            if (shiftKey) {
                addCount = 0;
                selMarkdown = selMarkdown.replace(/(^|\n)(\t| {1,3}[ \t])/g, function (match, start, indent) {
                    if (!addCount)
                        firstLineAddCount = -indent.length;
            ...
            setSelection(jThis, selection.start + firstLineAddCount, selection.end + addCount, selection.backward);
        }
        return false;
    }
}); });